...
static uint8_t next_byte(PatchApply *pa){
    ...
    if(!pull_raw(pa,&b)){ pa->g_body_left=0; pa->g_rcerr=1; return 0; }
    pa->g_body_left--;
    return b;
}
...
#define ULEB32_OVERFLOW(sh,b) ((sh)==28 && (uint8_t)(b)>15u)
...
static int env_uleb(PatchApply *pa, uint32_t*out, uint8_t*ov){
    ...
    do{ if(!env_byte(pa,&b) || ULEB32_OVERFLOW(sh,b)) return 0; v|=(uint32_t)(b&0x7fu)<<sh; sh+=7; n++; }while(b&0x80u);
...
static int rc_init(PatchApply *pa){
    ...
    for (int i=0;i<4;i++) pa->RC.code = (pa->RC.code<<8) | next_byte(pa);
    return !pa->g_rcerr;
}
...
static int s_bit_r(PatchApply *pa, uint16_t*prob,int rate){
    uint32_t p=*prob;
    int b=rc_decode(pa,RC_PROB_BOUND(pa->RC.range,p));
...
static int s_raw(PatchApply *pa){ return rc_decode(pa,pa->RC.range>>1); }
...
static uint32_t s_unary(PatchApply *pa, uint16_t*u,uint32_t clampmax,uint32_t cap){
    uint32_t cl=0; while(s_bit(pa,&u[cl<clampmax?cl:clampmax])==1){ if(++cl>cap){ pa->g_rcerr=1; return 0; } }
    return cl;
}
...
static void orow_commit_all(PatchApply *pa){
    for(uint32_t i=0;i<OUTROW_DEPTH;i++){
        ...
        for(uint32_t s=0;s<OUTROW_DEPTH;s++){
            ...
            if(best==OROW_NONE || (pa->g_FWD ? b<best : b>best)){ best=b; bs=s; }
        }
...
static int32_t pull_delta(PatchApply *pa, up_DRStream*d, up_IdxUnary*gix, int32_t*dic, uint32_t cap){
    ...
    if(s_bit(pa,&d->hit)==1){
        ...
        v=s_bv(pa,&pa->MDL_pre.dval, RC_DVAL_RATE);
...
static uint8_t next_content(PatchApply *pa, up_ApplyState*s, int tag){
    for(;;){
        ...
        if(s->tok_left>0){
            ...
            else if(s->tok_mode==2){ b=s->ring[s->tok_src & MASK]; s->tok_src++; }
            else { b=out_read(pa,s->tok_src); s->tok_src+=(uint32_t)(pa->g_FWD?1:-1); }
            if(pa->g_rcerr) goto fail;
            emit_ring(pa,s,b); s->tok_left--;
        ...
        if(!is_match){
            ...
            else { d=s_ug_rice(pa,&pa->MDL_tok.gd)+1u; pa->g_lastdist=d; }
            uint32_t ln=s_ug_gamma(pa,&pa->MDL_tok.gl)+1u;
            if(d==0 || d>s->ototal || d-1u>=RING) goto fail; /* reject before-start / ring-overrun */
...
static void hy_src_rec(PatchApply *pa, int32_t fp, uint8_t v){
    if(fp>=0 && (uint32_t)fp<pa->g_from_size){
        ...
        if(a&1u){
            if(pa->g_psrc_even==a-1u) hy_half_rec(pa,a-1u,pa->g_psrc_lo,v);
        } else {
            pa->g_psrc_lo=v;
            pa->g_psrc_even=a;
        }
 ...
 * two call sites share one body instead of duplicating the write loops inside apply_op. */
...
static void RC_NOINLINE litcur_step(PatchApply *pa, up_ApplyState*s, up_LitCur*lc, int32_t nl){
    if(lc->li<nl){
        ...
        lc->nextpos=(int32_t)np; lc->litb=next_content(pa,s,0);
...
static void RC_NOINLINE apply_op(PatchApply *pa, up_ApplyState*s){
    ...
    int corr=0;
    uint32_t n=s_ug_gamma(pa,&pa->MDL_pre.pgn);
    for(;;){
        uint32_t off=0;
    ...
    int32_t nl=(int32_t)read_uleb(pa,s);
...
/* object that patch_apply_run primes before invoking decode_body.                            */
/* ===================================================================================== */
...
static int RC_NOINLINE decode_header(PatchApply *pa){
    ...
    int32_t fp_start=0;
    if(!env_u32le(pa,&want_from) || !env_u32le(pa,&want_to) || !env_uleb(pa,&fs,0) || fs>MAX_IMAGE || !env_uleb(pa,&z,&ov)) return 0;
...
static int decode_body(PatchApply *pa){
    ...
    { uint32_t mn=s_ug_gamma(pa,&pa->MDL_pre.gdl);
      if(mn>SMAP_CAP){ pa->g_reject=REJ_RESOURCE; return 0; }
      uint32_t b=0;
    ...
    rc_dr_init(&pa->DR_BL, pa->DR_DIC_BL, DR_HIT_INIT); rc_dr_init(&pa->DR_EX, pa->DR_DIC_EX, DR_HIT_INIT);
    ...
    { up_ApplyState*s=&pa->ARENA.apply.sa;
       ...
       * until tp reaches 0. This is SAFE because apply_op REJECTS any zero-output op (nw==0), so
      ...
      while(!pa->g_rcerr && (pa->g_FWD ? s->tp!=(int32_t)pa->g_to_size : s->tp!=0)) apply_op(pa,s);
      /* tp must land exactly; the loop guarantees it on the clean path (it exits only on g_rcerr or
       * tp==target, and apply_op's frontier guards make overshoot impossible), so no post-loop tp
