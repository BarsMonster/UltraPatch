 ...
 *
 * Host encoder module -- field classification, shift-map fitting, body assembly, and
 * range-coder emission: op_emit_content, emit_delta, emit_geom, emit_body, encode_body.
 * Compiled as a normal internal encoder translation unit.
 */

#include "enc_internal.h"

static int field_addr(int32_t fp0, int32_t k, uint32_t size, uint32_t *out) {
    int64_t a = (int64_t)fp0 + k;
    if (a < 0 || a + 4 > (int64_t)size) return 0;
    *out = (uint32_t)a;
    return 1;
}
...
static int ldr_target_index_query(const LdrTargetIndex *idx, int32_t fp0,
                                  int32_t dl, uint32_t fpk) {
    if (!rc_ldr_target_in_op(fp0, dl, fpk)) return 0;
    uint16_t back = idx->back[fpk >> 2];
    return back && (int32_t)(fpk - back) >= fp0;
}
 ...
 * This is the encoder mirror of the decoder apply_op window skeleton. FWD/reverse asymmetry is
 * preserved exactly:
...
static void op_emit_content(const Op *o, int FWD,
    ...
    size_t inj0 = out->n;
    IVec lits = {0};
    int32_t walk_k = FWD ? 0 : o->diff_len - 1;
...
Buf encode_body(const EncCtx *ctx, const OpVec *ops, const uint8_t *frm, uint32_t from_size,
    ...
    TokenVec seq = lz_candidates_c(content.d, tags.d, content.n, lit->L0, lit->L1,
                                   &kd, &cands, &ncand);
    ...
    EmitBodyMeasure meas = { ops, &lit->seeds, &content, &tags, rows, FWD };
    ...
    Buf body = emit_body(&seq, kd, ko, ops, FWD, &lit->seeds, &content, &tags, rows,
